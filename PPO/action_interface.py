...
class Actuator:
    ...
    def _select(topleft, botright):
        ...
        tl = np.array(topleft) * (Actuator._SCREEN / (Actuator._SELECT_SPACE-1))
        br = np.array(botright) * (Actuator._SCREEN / (Actuator._SELECT_SPACE-1))
        
        tl_transform = Actuator._screen_normalize(tl)
        br_transform = Actuator._screen_normalize(tl)
